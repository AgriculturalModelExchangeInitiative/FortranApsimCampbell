...
CONTAINS
    ...
    FUNCTION interpolateTemperature(timeHours, &
        ...
        IF(time .LT. minT_time) THEN
            midnightT = SIN((0.0 + 0.25 - maxT_time) * 2.0 * 3.1415926535897931) *  &
        ...
        ELSE
            currentTemperature = SIN((time + 0.25 - maxT_time) * 2.0 *  &
                    3.1415926535897931) * (weather_MaxT - weather_MinT) / 2.0 + weather_MeanT
            res_cyml = currentTemperature
            RETURN
        END IF
    ...
    SUBROUTINE doNetRadiation(solarRadn, &
        ...
        TSTEPS2RAD = Divide(2.0 * 3.1415926535897931, REAL(ITERATIONSperDAY), REAL(0))
        solarConstant = 1360.0
        solarDeclination = 0.3985 * SIN((4.869 + (clock_Today_DayOfYear * 2.0  &
                * 3.1415926535897931 / 365.25) + (0.03345 * SIN((6.224 +  &
                (clock_Today_DayOfYear * 2.0 * 3.1415926535897931 / 365.25))))))
        ...
        DO timestepNumber = 1 , ITERATIONSperDAY + 1-1, 1
            m1(timestepNumber+1) = (solarDeclination * SIN(weather_Latitude *  &
                    3.1415926535897931 / 180.0) + (cD * COS(weather_Latitude * 3.1415926535897931 /  &
    ...
    FUNCTION calcSoilTemperature(soilTempIO, &
        ...
        IF(weather_Latitude .GT. 0.0) THEN
            offset = -0.25
        END IF
        print *, "climat", "Latitude", weather_Latitude, "offset", offset, "weather_Tav", &
                weather_Tav, "weather_Amp", weather_Amp
        
        print *, "cumulativeDepth", cumulativeDepth
        print *, "zd", zd, "w", w, "dh", dh, "offset", offset
        print *, "clock_Today_DayOfYear", clock_Today_DayOfYear, &
                "weather_Amp", weather_Amp, "weather_Tav", weather_Tav
